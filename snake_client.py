    ...
    while True:
        try:
            ...
            if not data:
                print("Disconnect")
                break
            if not data.startswith(b'('):  # it is a message not game state
                # Handling chat message
                decrypted_chat_message = private_key.decrypt(data,
                                                             padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()),
                                                                          algorithm=hashes.SHA256(),
                                                                          label=None)
                                                             ).decode()
                decrypted_chat_message = decrypted_chat_message[4:]  # Remove 'msg:' prefix
                print(f"{decrypted_chat_message}")
