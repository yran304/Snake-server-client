...
# Upon Connection, send client's public key to server
...
def redraw_window(surface, snakes_info, snacks_info):
    ...
    for snake_info in snakes_info:
        if snake_info:  # Ensure there is at least one segment of the snake
            ...
            snake_color = rgb_colors_list[np.random.randint(0, len(rgb_colors_list))]
    ...
    while True:
        try:
            ...
            if not data:
                print("Disconnect")
                break
            if data.startswith(b'msg:'):  # it is a message not game state
                # Handling chat message
                encrypted_chat_message = data[4:]  # Remove 'msg:' prefix
                print(encrypted_chat_message)  # -------------------------can print until this line
                try:
                    decrypted_chat_message = private_key.decrypt(encrypted_chat_message,
                                                                 padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()),
                                                                              algorithm=hashes.SHA256(),
                                                                              label=None)
                                                                 )  # -----------------this should be the problem
                    print(f"{decrypted_chat_message}")  # ---------------------this line not printed!!!!!!!
                except Exception as e:
                    print("Message not correctly decrypted", e)
                    continue
            else:  # Parse the game state received from server
                parse_state = parse_game_state(data.decode())
                if len(parse_state) == 2:
                    snakes, snacks = parse_state
...
def send_command(command):  # everything sent to server need to be encrypted
    # print('XXX', command)
    try:
        # Encrypt using server's public key
        encrypted_command = server_public_key.encrypt(command.encode(),
                                                      padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()),
